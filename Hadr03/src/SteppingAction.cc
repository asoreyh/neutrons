...
SteppingAction::SteppingAction()
  : G4UserSteppingAction(), gammas(0), gammasec(0)
{ }
...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
 ...
 const G4StepPoint* endPoint = aStep->GetPostStepPoint();
  ...
  G4String PartType = track->GetDefinition()->GetParticleType();
  G4String PartSubType = track->GetDefinition()->GetParticleSubType();
  ...
  if (transmit) {
    if(PartName != "gamma")
      G4cout << "Par-MAL" << "= " << PartName << G4endl;
  ...
  //G4cout << "NUM SECONDARIES" << "= " <<  (*secondary).size() << G4endl;
